		...
		xhr.onreadystatechange=function(){		//最后监测状态改变，进行服务器回应的处理
			if(xhr.readyState==4){
				if(xhr.status==200){
					...
					alert("coupon ok?"+(coupon==null||coupon==undefined));
...
$("#commitButton").click(function(){
	if(buyNowGoodInfo!=""){//buynow
		...
		alert("ojson address="+nowaddressid);
		...
		xhr.onreadystatechange=function(){		//最后监测状态改变，进行服务器回应的处理
			if(xhr.readyState==4){
				if(xhr.status==200){
					...
					//转至cashier
					window.location.href="http://localhost:8001/htmls/cashier1.html?param="+orderformId;
				}else{
					alert("status error");
				}
			}
