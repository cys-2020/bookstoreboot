...
$(function(){
	// 获取省
		...
		// 循环第二步,把所有的市都循环进select
			provlevel.onchange = function () {
				...
				citylevel.options[citylevel.length] = new Option("城市");
				arealevel.options.length = 0;
				arealevel.options[arealevel.length] = new Option("区/县");
				for (var j = 0; j < city[provlevel.selectedIndex - 1].cityList.length; j++) {
					citylevel.options[citylevel.length] = new Option(city[provlevel.selectedIndex - 1].cityList[j].name)
				}
			};
			citylevel.onchange = function () {
				arealevel.options.length = 0;
				arealevel.options[arealevel.length] = new Option("区/县");
				for (var k = 0; k < city[provlevel.selectedIndex - 1].cityList[citylevel.selectedIndex - 1].areaList.length; k++) {
					arealevel.options[arealevel.length] = new Option(city[provlevel.selectedIndex - 1].cityList[citylevel.selectedIndex - 1].areaList[k]);
				}
			};
	...
	xhr.open("POST","../getAddressesList",true);		//然后指定服务器url
	...
	xhr.onreadystatechange=function(){		//最后监测状态改变，进行服务器回应的处理
		if(xhr.readyState==4){
			if(xhr.status==200){
				addressesList=xhr.response;
				var len=addressesList.length;
				for(var i=0;i<len;i++){
					$("#addressesListArea").append("<div id=\""+i+"\"></div>");
					if(addressesList[i].flag==1){
...
function showDefaultAddress(i){
	$("#"+i).html("<span id=\"setDefault\""+i+">默认</span>   "
	+addressesList[i].who+"<br>"
	+addressesList[i].address+addressesList[i].addressDetail);
	$("#setDefault"+i).addClass("defaultAddress");
	$("#setDefault"+i).click(function(){
		alert("单击了，吗");
		if(!$(this).hasClass("defaultAddress")){
			var param=addressesList[i].id;
			alert(param);
			var xhr=new XMLHttpRequest();		//首先新建一个xmlhr对象
			xhr.open("POST","../setDefaultAddress",true);		//然后指定服务器url
			xhr.responseType="json";
			xhr.setRequestHeader(
				"Content-Type", "application/json;charset=utf-8");//设置post请求头
			xhr.send(param);
...
function showCommonAddress(i){
	$("#"+i).html("<span id=\"setDefault\""+i+">设为默认</span>   "
	+addressesList[i].who+"<br>"
	+addressesList[i].address+addressesList[i].addressDetail);
	$("#setDefault"+i).click(function(){
		alert("单击了，吗");
...
function resetDefaultAddress(i){
	$("#setDefault"+i).addClass("defaultAddress")
	.siblings().removeClass("defaultAddress");
}
...
$("#removeAddressesButton").click(function(){
	var removedIndex=[];
	var addressIds=[];
	for(var i=0;i<addressesList.length;i++){
		if($("#"+i).hasClass("selected")){//i==>list index
			addressIds.push(addressesList[i].id);
			removedIndex.push(i);
		}
	}
	var param=JSON.stringify(addressIds);
	alert(param);
	var xhr=new XMLHttpRequest();		//首先新建一个xmlhr对象
	xhr.open("POST","../removeAddresses",true);		//然后指定服务器url
	xhr.responseType="json";
	xhr.setRequestHeader(
		"Content-Type", "application/json;charset=utf-8");//设置post请求头
	xhr.send(param);
	xhr.onreadystatechange=function(){		//最后监测状态改变，进行服务器回应的处理
		if(xhr.readyState==4){
			if(xhr.status==200){
				//显示删除后的list
				$("#addressesListArea").html("");
				for(var i=0;i<removedIndex.length;i++){
					addressesList.splice(removedIndex[i],1);
				}
				var len=addressesList.length;
				for(var i=0;i<len;i++){
					$("#addressesListArea").append("<div id=\""+i+"\"></div>");
					$("#"+i).html(addressesList[i].who+"<br>"+addressesList[i].address+addressesList[i].addressDetail);
				}
			}else{
				alert("status error");
			}
...
$("#ifDefault").click(function(){
	$(this).addClass("selected");
});
$("#addAddressButton").click(function(){
	var defaultAddressFlag=0;
	if($("#ifDefault").hasClass("selected")){
		defaultAddressFlag=1;
	}
	var address={
		who: $("#whoText").val(),
		phoneNumber: $("#phoneNumberText").val(),
		address:$("#provlevel option:selected").val()+
			$("#citylevel option:selected").val()+
			$("#arealevel option:selected").val(),
		addressDetail:$("#detailText").val(),
		flag:defaultAddressFlag
	};
	var param=JSON.stringify(address);
	alert(param);
	var xhr=new XMLHttpRequest();		//首先新建一个xmlhr对象
	xhr.open("POST","../addAddress",true);		//然后指定服务器url
	xhr.responseType="json";
	xhr.setRequestHeader(
		"Content-Type", "application/json;charset=utf-8");//设置post请求头
	xhr.send(param);
	xhr.onreadystatechange=function(){		//最后监测状态改变，进行服务器回应的处理
		if(xhr.readyState==4){
			if(xhr.status==200){
				...
				var i=addressesList.length;
				alert("当前地址个数"+i);
				alert(addressesList[0].who+","+addressesList[1].who
				+",,,"+addressesList[i-1].who);
				$("#addressesListArea").append("<div id=\""+(i-1)+"\"></div>");
				if(address.flag==0){
					showCommonAddress(i-1);
				}else{
					showDefaultAddress(i-1);
				}
			}else{
				alert("status error");
			}
