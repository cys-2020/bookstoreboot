...
$("#removeAddressesButton").click(function(){
	//needed indexes
	var removeIndexArr=[];
	var needRemoveIndexInList=new Array(5);
	for(var i=0;i<addressesList.length;i++){
		if($("#"+i).hasClass("selected")){//i==>list index
			//list(i).code==> needed index
			needRemoveIndexInList.push(i);
			alert("需要删除code为"+addressesList[i].code);
			removeIndexArr.push(parseInt(addressesList[i].code) );
		}
	}
	var param=JSON.stringify(removeIndexArr);
	alert(param);
	var xhr=new XMLHttpRequest();		//首先新建一个xmlhr对象
	...
	xhr.onreadystatechange=function(){		//最后监测状态改变，进行服务器回应的处理
		if(xhr.readyState==4){
			if(xhr.status==200){
				...
				for(var i=0;i<needRemoveIndexInList.length;i++){
					addressesList.splice(needRemoveIndexInList[i],1);
				}
				var len=addressesList.length;
				for(var i=0;i<len;i++){
					addressesList.append("<div id=\""+i+"\"></div>");
					$("#"+i).html(addressesList[i].value);
				}
			}else{
				alert("status error");
...
$("#addAddressButton").click(function(){
	...
	xhr.onreadystatechange=function(){		//最后监测状态改变，进行服务器回应的处理
		if(xhr.readyState==4){
			if(xhr.status==200){
				var newcode=parseInt(xhr.response);
				$("#addressesListArea").append("<div id=\""+addressesList.length+"\"></div>");
				$("#"+addressesList.length).html(param);
				addressesList.push(new address(newcode,param));
			}else{
				alert("status error");
			}
		}
